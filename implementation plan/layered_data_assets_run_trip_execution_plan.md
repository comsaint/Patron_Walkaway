...
| ✅ | **LDA-E1-05** | `run_day_bridge`：跨日 run 影響範圍 | Data Platform | E1-03 | `layered_data_assets/run_day_bridge_v1.py`、`scripts/materialize_run_day_bridge_v1.py`、`l1_paths.l1_run_day_bridge_partition_dir`、`schema/examples/manifest_run_day_bridge_l1_example.json`；輸出 `run_day_bridge.parquet`（`bet_gaming_day` 分區鍵，SSOT §5.2） | 對任意 `bet_gaming_day` 分區可列出該日有 bet 之交會 `run_id`；跨日 run 於相關多日分區皆可掃到（單元測試） |
