...
| 狀態 | Task ID | 任務 | Owner | 依賴 | 輸出 artifact | DoD |
| :---: | :--- | :--- | :--- | :--- | :--- | :--- |
| ⬜ | **LDA-E2-01** | `trip_fact`：3 個完整 `gaming_day` 關閉語義（實作可用「無 run」等價判定） | Data Platform | Phase 1 + **Phase 1R** | `trip_fact` | `trip_start_gaming_day` 分區；同時輸出 open/closed（`trip_end_*` 可 null + `is_trip_closed`）；MVP 採 full snapshot 重算（`player_id` 分桶）且不引入外部日曆表；SSOT「無 bet」fixture 與「無 run」等價判定一致性報告通過；`trip_fact` manifest 列舉觸及之 `run_end_gaming_day` 分區；日編排後續沿用 **E1-09** state 契約 |
| ⬜ | **LDA-E2-02** | `trip_run_map` membership | Data Platform | E2-01 | `trip_run_map` | trip→run 完整可重建 |
| ⬜ | **LDA-E2-03** | `trip_id` hash：§4.1 `first_run_id` + `source_snapshot_id` 錨定 | Data Platform | E2-01 | ID 規則單元／整合測試 | 同 snapshot 重跑 trip_id 不變；trip 關閉後補寫 `trip_end_*` 不得改變 trip_id |
