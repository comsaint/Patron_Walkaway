...
> ``bet_duckdb_window``、``run_state_machine``、``player_snapshot_asset``（與 legacy 互為鏡像；**同一軌請只保留一側候選**，否則靜態載入會 ``ValueError``）。
...
> - **Phase 3（移除條件）**：無外部工具依賴舊 metric / 舊 ``track`` 字串後，可刪除 legacy 欄位與 ``compute_bet_duckdb_window_features`` 等薄包裝（須 bump schema 版本並跑全鏈路 smoke）。
