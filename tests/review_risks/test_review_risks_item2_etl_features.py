...
§2：候選 spec 僅 **`trainer/feature_spec/feature_candidates.yaml`**（dev SSOT）；**不得**再存在 `trainer/features/feature_spec/feature_spec.yaml`（已移除冗餘拷貝）。
...
class TestFeatureSpecSingleSourceContract(unittest.TestCase):
    ...
    def test_canonical_feature_spec_yaml_exists(self):
        """trainer/feature_spec/feature_candidates.yaml must exist (dev SSOT)."""
        p1 = REPO_ROOT / "trainer" / "feature_spec" / "feature_candidates.yaml"
        self.assertTrue(
            p1.is_file(),
            "Canonical candidates must exist at trainer/feature_spec/feature_candidates.yaml (STATUS Code Review 2.2 etl/features §2).",
        )
    ...
    def test_redundant_features_subpackage_yaml_absent(self):
        ...
        self.assertFalse(
            p2.exists(),
            "Redundant trainer/features/feature_spec/feature_spec.yaml must not exist; edit trainer/feature_spec/feature_candidates.yaml only (STATUS Code Review 2.2 etl/features §2).",
        )
