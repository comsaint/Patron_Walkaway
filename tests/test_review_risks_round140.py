...
from trainer.trainer import ensure_player_profile_daily_ready, run_pipeline
...
class TestR200SchemaHashHorizonGuardrail(unittest.TestCase):
    ...
    def test_max_lookback_days_change_should_invalidate_existing_profile_cache(self):
        with TemporaryDirectory() as td:
            ...
            local_dir = Path(td)
            profile_path = local_dir / "player_profile_daily.parquet"
            session_path = local_dir / "gmwds_t_session.parquet"
            sidecar_path = local_dir / "player_profile_daily.schema_hash"
