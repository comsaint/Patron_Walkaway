...
def test_insufficient_fold_count_respects_cli_threshold(tmp_path: Path) -> None:
    """``insufficient_fold_count`` counts folds with ``0 < t_feasible_size < min`` (not empty folds)."""
    fold1 = tmp_path / "fold1.json"
    fold2 = tmp_path / "fold2.json"
    out_json = tmp_path / "precondition.json"
    out_md = tmp_path / "precondition.md"
    # Each fold: four non-null recall thresholds → t_feasible_size=4; min=5 → both insufficient.
    four_thr = {
        "test_positives": 100,
        "test_samples": 1000,
        "test_neg_pos_ratio": 9.0,
    ...
    assert output["t_feasible_stats"]["insufficient_fold_count"] == 2
