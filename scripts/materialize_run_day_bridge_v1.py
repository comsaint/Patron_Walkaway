#!/usr/bin/env python3
"""L1 ``run_day_bridge`` (LDA-E1-05): ``bet_gaming_day`` → affected ``run_id`` rows.

Same gap / ``run_id`` rules as ``materialize_run_fact_v1.py``. Writes
``run_day_bridge.parquet`` + ``manifest.json`` under
``data/l1_layered/<source_snapshot_id>/run_day_bridge/bet_gaming_day=.../``.
"""
...
from layered_data_assets.l1_paths import l1_run_day_bridge_partition_dir  # noqa: E402
...
from layered_data_assets.run_day_bridge_v1 import (  # noqa: E402
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """CLI for ``materialize_run_day_bridge_v1``."""
    ...
    p.add_argument(
        "--bet-gaming-day",
        required=True,
        help="Partition label: list run_id for runs that have ≥1 bet with this gaming_day (match cleaned t_bet)",
    )
    p.add_argument(
        "--l1-preprocess-gaming-day",
        required=True,
        help="Upstream cleaned-bet partition id for manifest source_partitions (often same as --bet-gaming-day)",
    )
    p.add_argument(
        "--input",
        dest="inputs",
        ...
        type=Path,
        required=True,
        help="Cleaned bet Parquet (repeatable); same inputs as run_fact / run_bet_map",
    )
    p.add_argument(
        "--l0-fingerprint-json",
        type=Path,
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override output directory (default: data/l1_layered/<snap>/run_day_bridge/bet_gaming_day=...)",
    )
    p.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    """Materialize one ``run_day_bridge`` partition."""
    try:
        import duckdb
    except ImportError:
    ...
    if out_dir is None:
        out_dir = l1_run_day_bridge_partition_dir(
            args.data_root.resolve(), args.source_snapshot_id, args.bet_gaming_day
        )
    out_parquet = out_dir / "run_day_bridge.parquet"
    ...
    stats, id_summary = run_duckdb_job_with_oom_retries(
        ...
        work=_work,
        input_paths=inputs,
        job_name="materialize_run_day_bridge_v1",
    ...
    manifest = build_run_day_bridge_manifest(
