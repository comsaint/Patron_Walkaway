...
player-game ready-queue dry-run disabled (shadow scoring left as-is).

See `scripts/run_scorer_once_disabled_pg.py` for original behavior.

Run from project root (cmd.exe):
    set PYTHONUNBUFFERED=1 && python -u scripts\run_scorer_once_disable_pg_ready_queue_only.py
"""
...
try:
    ...
    # Proxy: disable only the ready-queue dry-run
    class _ProxyCfg:
        def __init__(self, base):
            self._base = base

        def __getattr__(self, name):
            if name == "player_game_ready_queue_dry_run_enabled":
                return False
            return getattr(self._base, name)

    cfg_mod = _ProxyCfg(cfg_base)
    set_hightier_serving_deploy_override(cfg_mod)
    print("Set bundle-local deploy serving override (proxy) with player_game ready-queue dry-run disabled")
    ...
    argv = [
        ...
        str(allowlist),
        "--dry-run-report",
        str(BUNDLE_ROOT / "local_state" / "scorer_dry_run_report_readyq_disabled.json"),
    ]
