...
def _run_lda_pipeline_for_day(
    ...
    *,
    ...
    _run_step(pre_cmd, label=f"preprocess {d}", step=f"{d} preprocess")
    ...
    _run_step(rf_cmd, label=f"run_fact {d}", step=f"{d} run_fact")
    ...
    _run_step(bm_cmd, label=f"run_bet_map {d}", step=f"{d} run_bet_map")
    ...
    _run_step(br_cmd, label=f"run_day_bridge {d}", step=f"{d} run_day_bridge")
    ...
    for art in _GATE1_ARTIFACTS:
        ...
        _run_step(g_cmd, label=f"gate1 {art} {d}", step=f"{d} gate1-{art}")
