...
- **Deploy + SQLite 啟動**：scorer／validator 併發時可能出現 `database is locked`；延後 validator 至 scorer 首輪完成之計畫見 **Task 10**。
...
- **Goal**:
  ...
  2. **Validator KPI（SLO / 即時監控）**：主控台 **15 分鐘／1 小時**滾動窗之 `[validator] Cumulative Precision (…, by validated_at)`；`validator_metrics` 與該 KPI **同一套時間語意**（**2026-03-25 決策修訂**：以 **`validated_at`** 作為落窗參考時間，避免使用 `bet_ts`/`alert_ts` 導致驗證通常發生於事件很久之後、15m/1h 長期 `0/0` 而失去監控價值；長時間窗 precision 由 backtest 提供）。
  - **實作狀態（2026-03-25）**：已在 `trainer/serving/validator.py` 落地（含 tz-naive / mixed-tz `validated_at` 邊界處理），review_risks 測試已覆蓋。
...
- **Validator — Round 2（2026-03-25）已完成 `INFO` → `DEBUG`（商業取向）**：
...
- **Validator — 建議保留 `INFO`（商業／結果導向）**:
  ...
  - `This cycle: N alert(s) verified — ...`（本輪驗證摘要，便於商業使用者理解「剛剛發生了什麼」）
...
- **Behavior change（validator_metrics / SLO）**: 先前 `validate_once` 對 **所有** 已 finalize 列（retention 內）算一個 precision 並寫表；日誌卻標為「15m window」。後續調整為依 **參考時間**落於最近 15 分鐘／1 小時（HK，`[now−window, now]`）之子集計算 precision，並以**同一套定義**寫入 `validator_metrics`；**參考時間**採 **`validated_at`**（驗證完成時間）以支援即時監控；**`1h`** 僅多一條 `logger.info`，不另寫第二筆列（避免 schema 變更）。
- **Follow-up（2026-03-26）**：若實作將 **`now` 固定為 `validate_once` 週期開頭**，而 **`validated_at` 為每筆驗證當下**，則上界過緊可致**首輪／同輪** Cumulative Precision **`(0/0)`** 與「本輪 verified」並存。**已規劃修復**：滾動 KPI 上界改為**週期結束時刻** — 見 **Task 11**；決策紀錄 **DEC-038**。
- **Files**:
  - [trainer/serving/validator.py](../../trainer/serving/validator.py) — 新增/改用 `_rolling_precision_by_validated_at`（或等價實作）；雙行 Cumulative Precision log（標註 `by validated_at`）；`_append_validator_metrics` 與 15m SLO 對齊。
...
- **DoD**:
  ...
  - **商業取向第二輪（2026-03-25）**：預設 `INFO` 下 console 僅保留：
    - **A1/A2/A3**：Validator 的 15m/1h precision + 本輪 verified 摘要
    - **B1/B2**：Scorer 的 `Emitted N alerts` + 啟動 `Loaded model v=...`
    - Deploy 啟動 `print`（state/model/api 端點）
- **Incident follow-up（2026-03-25）— KPI under-count（保留 cache 路線）**：
  - **Incident 記錄**：見 [`INCIDENT.md`](INCIDENT.md)（`Validator Rolling KPI Under-count`）。
  - **Root cause**：`load_existing_results_incremental` 依 rowid watermark 增量載入，但呼叫端每輪以空 dict 起跑，導致 KPI 基底退化為「少量新列」而非完整狀態，進而出現 15m/1h 分母偏小、且常相同。
  - **Decision**：不改為每輪全表重查，採「**保留 cache** + **初始化 full bootstrap** + **後續增量**」。
  - **Execution plan**：
    1. **Correctness（立即）**：空 cache 啟動時強制 full bootstrap；loop 維持 process-level cache；KPI 仍以 `validated_at` 落窗計算。
    2. **Performance（短期）**：增加 cache size/每輪耗時 DEBUG 觀測，必要時按 retention 修剪 cache 舊鍵。
    3. **Regression（短期）**：新增「watermark + 空 cache 啟動」、「第二輪增量等價全量」、「15m/1h 分母不長期同值」測試。
  - **Follow-up DoD**：
    - 重啟後 2-3 輪內可觀測 `total_15m <= total_1h`（通常嚴格小於），且兩者不再長期同值。
    - 15m KPI 分母與 `validation_results` 同窗抽查量級一致。
    - `validator_metrics` 保持為 15m 快照監控用途，不用於跨列重加總回算事件 precision。
...
### Task 9 — Validator「無 bet data 長期 PENDING」修正（方案 1：最舊待驗時間窗）— 🚧 **In progress**（2026-03-25）
...
- **Implementation**:
  - [trainer/core/config.py](../../trainer/core/config.py)
    - 新增：
      ...
      - `VALIDATOR_FETCH_MAX_LOOKBACK_MINUTES_CAP`（int，default 1440；硬上限避免誤設造成 CH runaway）
    - 加入合理邊界與 fallback（非法值回預設），並對過大 lookback 做 clamp（warn-once）。
  - [trainer/serving/validator.py](../../trainer/serving/validator.py)
    ...
    - 加固：
      - safe int 解析 helper（避免非法配置炸整輪）
      - misconfig warning warn-once（避免洗版）
      - `required_min` 納入 `VALIDATOR_EXTENDED_WAIT_MINUTES`（避免低估所需回看窗）
...
- **Status / Remaining items（Task 9）**:
  - ✅ 已完成：動態 fetch window（最舊待驗 + pre_context + hard_floor）、配置防呆、lookback cap、warn-once、tests/typecheck/lint 綠燈（細節見 `STATUS.md`）。
  - ⏳ 待完成：
    - 明確決策「45–47 分鐘定案」與 `VALIDATOR_EXTENDED_WAIT_MINUTES=15` 的最終取捨（保留 60m 或 clamp 回 47m），並同步文件/參數名。
    - 加入每輪 `No bet data` 次數/比例（建議先 `DEBUG`）以量化調參效果。
    - 依 production log 做調參 runbook（建議範圍、遇到積壓/flush 的操作方式）。

- **Task 9B（新增）— `No bet data` 二階段補查（targeted retry）**:
  - **目的**：維持「truth-first」（不把未知直接收斂成 MISS），針對主查詢 miss 的少量 alerts 做精準補查。
  - **觸發條件**：本輪 `validate_alert_row` 回傳 `No bet data ... leaving PENDING` 的 alerts。
  - **補查策略**：
    - 第 1 階段：保留現有 batch `fetch_start/fetch_end` 查詢。
    - 第 2 階段（補查）：僅對 miss alerts 送 dedicated query，並回填 `bet_cache` 後重跑該批 alerts 一次。
  - **補查窗定義（依你要求）**：
    - 每筆 alert 使用自身 `bet_ts`（即該筆 `payout_complete_dtm`）作為時間基準。
    ...
    - 不再用全域 `pending_min_ts`/`hard_floor` 裁切這批補查。
  - **上限與成本控制**：
    - 每輪補查上限：`VALIDATOR_NO_BET_RETRY_MAX_ALERTS = 50`（超過則本輪只處理前 50，剩餘留待下一輪）。
    - 仍沿用 player_id chunking；僅查必要欄位（`player_id`, `payout_complete_dtm`）。
    - 補查只做一次（每輪每筆最多一次），避免 N+1 連鎖查詢。
  - **觀測（DEBUG）**：
    - `no_bet_before_retry`, `retry_selected`, `retry_resolved`, `retry_rows`, `retry_query_count`。
    - 每筆補查 log：`bet_id/canonical_id/player_id/retry_start/retry_end/rows_returned`（必要時做採樣，避免洗版）。
  - **DoD（Task 9B）**：
    - 既有「2h 舊 bet_ts、alert 新」案例在補查後可取得 bet 序列並停止長期重複 `No bet data`。
    - 補查機制不影響主查詢 latency（無 miss 時不觸發第 2 階段）。
    - 在壓力情境下（大量 miss）仍可受 `50/輪` 上限保護，不造成 CH 突發高負載。
  - **測試建議**：
    - 單元：miss -> 補查命中 -> `bet_cache` 回填成功，且同輪 re-validate 不再 `No bet data`。
    - 單元：`No bet` > 50 筆時僅取 50 筆補查，剩餘留下一輪。
    - 整合：mock CH 驗證 retry 窗使用該筆 `bet_ts` 而非全域 `pending_min_ts`。
  - **Status / Remaining items（Task 9B）**：
    - ✅ 已完成：targeted retry 主流程、每輪上限 `50`、以單筆 `bet_ts` 為基準窗、回填 `bet_cache` 後同輪重驗、全量 tests/lint 與 focused mypy 綠燈（見 `STATUS.md`）。
    - ✅ **Round 2（2026-03-25）**：per-player `query_df` 容錯（`failed_queries`）、`VALIDATOR_NO_BET_RETRY_MAX_WINDOW_MINUTES` 單筆／聚合窗 clamp、**round-robin** 取 `retry_limit`（避免固定前 N 飢餓）、`validate_once` **DB 優先** 與 `warm_cache` 合併語意、契約測試（`test_task9b_retry_review_risks_mre.py`、`test_validator_phase2_incremental_review_risks_mre.py`）對齊。
    - ⏳ 可選後續：
      - `>50` case 的額外整合測試（若需更強回歸）。
      - production 調參與 `failed_queries` 監控儀表。
...
### Task 9C — Validator：No-bet 補查以 `bet_id` 錨定 TBET（方案 C）— ✅ **Done（MVP）**（2026-03-26）

- **Incident 記錄（僅敘事與證據）**：見 [`INCIDENT.md`](INCIDENT.md) 第二則 — **Validator「No bet data」與 TBET `player_id` 漂移**。
- **Status**：MVP 已落地（no-bet retry 路徑內 chunked `bet_id IN`、合併 `bet_cache`、retry summary 含 `bet_id_*` 欄位）。**未做**：主 fetch 路徑預先 bet_id batch；CH `player_id` 與 alert 不同時之**第二輪窄窗**補拉（Design notes 第 2 點）。
- **Context（摘要）**：
  - Task 9 / 9B 已用「最舊待驗 + hard_floor」主查詢與 **per-`player_id` + 時間窗** 的 targeted retry；在生產證據中，retry 窗口幾何上覆蓋 `bet_ts` 仍得 **`rows=0`**，而同筆 `bet_id` 曾於 scorer 週期自 TBET 讀出。
  - 調查結論：TBET（`FINAL`）下列內容會隨 ETL/merge 演進，**`player_id` 可能改寫**；**`bet_id` 視為穩定主鍵**（上線後仍應抽樣驗證）。
- **Goal**：
  1. 當 `player_id` + 時間窗查詢為空時，仍能以 **`bet_id`** 從 `TBET FINAL` 取回該注單的 `payout_complete_dtm`（與現有 filter：`wager > 0`、`payout_complete_dtm IS NOT NULL`、排除 placeholder 等 **對齊**）。
  2. 將取得時間戳 **合併**進現有 `bet_cache` 結構，使 `validate_alert_row` 不再因空列表誤判為「無注單資料」。
  3. 可觀測：`DEBUG` 統計（bet_id 查詢次數、命中列數、CH 例外）、可選擇性 log 單筆 `bet_id` 命中以利對帳。
- **Non-goals（本輪不做）**：
  - 重寫 canonical mapping 管線或 deploy 連續更新策略（長期項，見 INCIDENT「Long-term」）。
  - 改變 `LABEL_LOOKAHEAD` / gap 政策數學定義（除非評測發現單點注入不足）。
- **Design notes（重要）**：
  - **`bet_cache` 語意**：`validate_alert_row` 以 `canonical_id`（或 `str(player_id)`）鍵取 **排序後的 `payout_complete_dtm` 列表** 做 bisect 與 `find_gap_within_window`。  
    **僅注入該 alert 對應的一個時間戳**可解除「完全無資料」狀態，但在 **鄰近注單稀少**時，gap 結論可能與「該 player 完整時間序列」略有差異。  
  - **建議實作順序**：  
    1. **MVP**：`bet_id` → `payout_complete_dtm` → merge 進 `bet_cache`。  
    2. **可選加強（同一 PR 或 fast-follow）**：若 `bet_id` 列回傳的 **`player_id` 與 alert 上不同**，再以 **CH 回傳的 `player_id`** 追加一輪 **窄時間窗** `player_id` 查詢（與現有 retry 窗口對齊或略寬），合併完整列表，使 gap 語意接近原設計。
- **Implementation**：
  1. **[`trainer/core/config.py`](../../trainer/core/config.py)**（若需要）  
     - 新增可選常數，例如：`VALIDATOR_NO_BET_BET_ID_LOOKUP_ENABLED`（預設 `True`）、`VALIDATOR_BET_ID_CHUNK_SIZE`（避免過大 `IN`），命名與現有 `VALIDATOR_*` 風格一致。
  2. **[`trainer/serving/validator.py`](../../trainer/serving/validator.py)**  
     - 新增 **`fetch_bet_payout_times_by_bet_ids`**（或等價名稱）：  
       - SQL：`SELECT bet_id, payout_complete_dtm[, player_id] FROM {DB}.{TBET} FINAL WHERE bet_id IN %(ids)s AND …`（與 `fetch_bets_by_canonical_id` / `_fetch_bets_for_no_bet_rows` 同一套 `wager`、`payout_complete_dtm`、`player_id` 條件對齊）。  
       - 時間欄位以 **HK_TZ** 正規化，與現有函式一致。  
       - 回傳結構：`Dict[bet_id_str, datetime]` 或附 `player_id` 供第二階段擴充。  
     - 在 **`_fetch_bets_for_no_bet_rows`** 末尾（或與現有 per-`player_id` 迴圈組合）：  
       - 對 `retry_slice`（或合併去重後的 bet_id 集合）做 **chunked** 查詢；  
       - 對每個命中列，依對應 alert 列的 `canonical_id` / `player_id` 將 `payout_complete_dtm` **加入** `cache` 中對應鍵（與現有 `win["cids"]` 邏輯一致：同一 `player_id` 多 alert 可共享）。  
     - **錯誤處理**：延續 Task 9B 精神——單次 bet_id 批次查詢 `try/except`，記 `failed_queries`，不整輪 abort。  
     - **日誌**：`DEBUG` 級別 `bet_id lookup: n_ids=… rows=…`；命中時可打 `bet_id` + `payout_complete_dtm` +（若有）CH `player_id` vs alert `player_id` 以便日後稽核 drift。
  3. **可選：主路徑**  
     - 若希望減少「第一輪就 No bet」：在 `bet_cache` 為空且 pending 量可接受時，對本輪 pending **bet_id** 做輕量 batch（需注意 QPS 與 `IN` 大小）；**建議先只做 no-bet retry 路徑**，觀測無誤再擴展。
  4. **測試**  
     - **單元 / 契約**：mock `get_clickhouse_client`，斷言在 `player` 查詢回空時，bet_id 補齊後 `bet_cache` 含預期時間戳、`validate_alert_row` 不再設 `_no_bet_data`。  
     - 更新或新增 `tests/review_risks/` 下與 Task 9B 相關的 MRE（字串錨點含 `bet_id` 查詢或函式名）。  
     - **Regression**：確保未觸發 no-bet 路徑時行為與 CH 呼叫次數無不當暴增（可抽樣計數 mock）。
  5. **營運驗證（上線後）**  
     - 對先前問題 `bet_id` 在 CH 手動：`SELECT bet_id, player_id, payout_complete_dtm FROM … FINAL WHERE bet_id = …` 確認與 alert 是否一致。  
     - 觀察 `No bet data` 警告率是否下降；若仍出現，檢查是否為 `bet_id` 缺失、TBET 延遲超過政策、或 filter 排除（`wager` 等）。
- **DoD**：
  - 問題路徑上，**在 `player_id` 查詢為 0 列時**，只要 TBET 仍存在該 `bet_id` 且通過 filter，validator **應能於當輪或下一輪**完成非 PENDING（或進入既有 gap 流程），不再無限 `No bet data`。  
  - 新增測試綠燈；`ruff` / `mypy` 通過。  
  - 本檔 Changelog、`STATUS.md` 追加本修復與手動驗證步驟。
- **Open questions**：
  - TBET 是否曾出現 **同一 `bet_id` 兩列**（`FINAL` 前後版本）導致多個 `payout_complete_dtm`：若存在，需定義 **取最新 / argMax** 規則（與表引擎約定對齊）。  
  - `bet_id` 在 SQLite/alert 中的型別（str/int）——查詢前 **統一正規化為 CH 側型別**。
- **Rollback**：
  - 還原 `fetch_bet_payout_times_by_bet_ids` 與 `_fetch_bets_for_no_bet_rows` 內之合併邏輯；config 常數可保留為 `False` 作為 feature off。

---

### Task 10 — Deploy：SQLite 啟動鎖競爭 — 延後 validator 至 scorer **首輪完成** — ✅ **Done**（2026-03-26）

- **Problem**：
  - `package/deploy/main.py` 以兩條 daemon thread **幾乎同時**啟動 `run_scorer_loop` 與 `run_validator_loop`，兩者共用 **`STATE_DB_PATH`**。
  - Scorer 首輪含 `init_state_db()`、長連線、`score_once` 內大量 SQLite 寫入／提交；Validator 於 `run_validator_loop` 開頭即 `get_db_conn()`，內含多項 **`CREATE`／`ALTER`／索引** DDL。
  - 預設 **`busy_timeout=0`** 時，短暫鎖競爭會拋 **`sqlite3.OperationalError: database is locked`**；錯誤可能自行消失，但污染啟動日誌並不利於 on-call 判讀。

- **Goal**：
  1. 維持 **單一 process + SQLite**（不引入第二顆 state DB、不改換引擎）。
  2. **Validator 執行緒**僅在 **scorer 完成第一個完整 `score_once` 週期之後**才開始執行 `run_validator_loop`（含其開頭的 `get_db_conn()` 與首輪 `validate_once`）。
  3. 語意澄清：**「首輪完成」= 第一次 `score_once(...)` 呼叫回傳後**（無論該輪內是否記錄 ERROR），**不要求** `alerts` 表已有列（安靜時段可能仍為 0 筆）。

- **Non-goals（本任務可不做，建議列為 fast-follow）**：
  - 全路徑統一 `PRAGMA busy_timeout`（仍建議另案補上，作為第二道防線，涵蓋執行期中 API 與 background 併讀寫）。
  - 以「`COUNT(*)` from alerts > 0」作為啟動閘門（會在長期無 alert 時 **餓死** validator）。

- **Design**：
  - 使用 **`threading.Event`**（例如 `scorer_first_cycle_done`），由 **scorer 執行緒**在 **第一次** `while` 迴圈迭代結束時 **`set()`**。
  - **建議在 `try`／`finally` 或迴圈尾**保證：即使 `score_once` 丟例外（現有行為為 log 後繼續），仍 **`set()`**，避免 validator **永遠不啟動**。
  - **Deploy 執行緒模型**：
    1. 建立 `Event`（初始 clear）。
    2. `threading.Thread(target=_run_scorer_with_signal, ..., kwargs)`：內部呼叫現有 `run_scorer_loop`，並傳入上述 Event（見 Implementation）。
    3. `threading.Thread(target=_run_validator_after_scorer, ...)`：開頭 `event.wait()`（可選 **`timeout`**，見 Open questions），再呼叫 `run_validator_loop(...)`。
  4. 啟動順序維持 **先 scorer、後 validator**（與現狀一致）；僅 validator 執行緒在 `wait` 上阻塞，**Flask `app.run`** 仍可立即起服務（與「API 早於 validator 第一輪」相容）。

- **Implementation**：
  1. **[`trainer/serving/scorer.py`](../../trainer/serving/scorer.py)** — `run_scorer_loop`  
     - 新增可選參數，例如 **`first_cycle_done: threading.Event | None = None`**（或等價名稱，型別註解與專案 Python 版本一致）。  
     - 在 **`while True:`** 第一輪迭代結束後（`score_once` 已返回、sleep 前後皆可，但須明確定義：**僅 set 一次**）呼叫 `first_cycle_done.set()`；若參數為 `None` 則 no-op。  
     - **`once=True`**（單次 CLI／測試）路徑：仍須在該單次迭代後 `set()`，以便 deploy 測試與對稱行為。
  2. **[`package/deploy/main.py`](../../package/deploy/main.py)**  
     - 建立 `Event`；`_run_scorer` 改為傳入 `first_cycle_done` 給 `run_scorer_loop`。  
     - 新增薄包裝 **`_run_validator_deferred(event)`**：`event.wait()` → `run_validator_loop(...)`。  
     - 啟動兩執行緒處更新；**主控台說明**可增一行：validator 於 scorer 首輪完成後啟動（避免維運誤判「validator 掛了」）。
  3. **[`deploy_dist/main.py`](../../deploy_dist/main.py)**（若與 deploy 主檔同步維護）— **同 diff**。
  4. **可選 env**：`DEPLOY_VALIDATOR_START_TIMEOUT_SECONDS`（或掛在既有 config）— `wait(timeout)` 逾時後 **log warning** 並仍啟動 validator，避免 CH／網路導致 scorer 首輪卡死時 **永久無驗證**（若產品決策為「寧可 locked 錯誤也要嘗試」則可省略，但建議文件化預設行為）。

- **DoD**：
  - 本機／staging：**連續多次**冷啟動 deploy，**不再**於啟動瞬間出現 validator 端 `database is locked`（或顯著減少至零；若仍偶發，應回頭補 **busy_timeout** 任務）。
  - Scorer **`once=True`** 單元／整合路徑：`Event` 仍被 set（可用 mock `score_once` 或輕量 stub DB 驗證）。
  - `ruff` / 相關測試綠燈；本檔與（若適用）[`STATUS.md`](STATUS.md) 標註 Task 10 **Done**。

- **Open questions**：
  - **`wait` 是否要有 timeout**：預設建議 **有**（例如 600s）並打 warning，避免營運上「validator 永不啟動」；若堅持無限等待，須在 README 註明。
  - **wheel／`walkaway_ml` 發佈**：若 scorer 簽名變更，需同步 **bump 套件版本**與 deploy `requirements.txt` 約束（依 repo 既有發佈流程）。

- **Rollback**：
  - 還原 `run_scorer_loop` 可選參數與 deploy 雙執行緒包裝；行為回到「兩執行緒同時啟動」。

- **Verification（2026-03-26）**：全量 `pytest tests/`、`ruff check .`、`mypy trainer/ package/` 綠燈；見 [`STATUS.md`](STATUS.md) Task 10「全 repo 驗證輪」。

- **Remaining / follow-ups（本 Task 之外或選做）**：
  1. **Fast-follow（PATCH 已列 Non-goal）**：凡連線 `STATE_DB_PATH`（scorer／validator／Flask `get_db_conn`）統一 **`PRAGMA busy_timeout`**，降低執行期與邊界情況下 **`database is locked`**；實作後需更新 Task 10 MRE 中「`get_db_conn` 無 `busy_timeout`」之靜態契約測。
  2. **Review #1**：`load_dual_artifacts`／連線／`init_state_db` 等在**進入首輪 `while` 前**失敗時，`first_cycle_done` 仍不會 set；validator 依 **逾時**或**無限 wait**解阻。可選 prod 修復：外層 `finally` set、或 deploy wrapper 捕未處理例外並 log + set（與 STATUS Code Review #1 對齊）。
  3. **Review #4**：極短 `DEPLOY_VALIDATOR_START_TIMEOUT_SECONDS`（如 &lt;5s）目前**無**警告；若產品要強制下限或 warning，另開小補丁並調整 MRE `#4` 靜態契約。
  4. **營運 DoD**：staging／prod **多次冷啟** deploy，確認啟動段 validator **`database is locked`** 顯著減少或消失（自動測無法取代）。
  5. **發佈**：`run_scorer_loop` 簽名變更若透過 **walkaway_ml wheel** 發佈，需 **bump 版本**並對齊 `package/deploy/requirements.txt`（見 Open questions）。

---

### Task 11 — Validator 滾動 KPI：`now` 上界錨定為「驗證週期結束時刻」（與 `validated_at` 一致）— ✅ **Done**（2026-03-26）

- **Context（觀測）**：
  - Deploy 下同一輪 log 先出現 **`[validator] This cycle: N alert(s) verified — …`**（`INFO`），緊接 **Cumulative Precision（15m / 1h）** 卻為 **`0.00% (0/0)`**，尤以**首輪**最明顯。
  - 根因（靜態分析）：`validate_once` 在週期**開頭**取 `now_hk = datetime.now(HK_TZ)`，後續整段週期（SQLite、ClickHouse、`validate_alert_row` 迴圈）**不再更新**；而 `_rolling_precision_by_validated_at` 以 **`[now_hk − window, now_hk]`** 篩 `validated_at`。
  - 相對地，`validate_alert_row` 內對每筆結果寫入的 **`validated_at`** 為該筆驗證當下的 **`datetime.now(HK_TZ)`**，通常 **晚於** 週期開頭的 `now_hk`。因此 **`validated_at <= now_hk`（週期起點）** 不成立，本輪新 finalize 列全被濾除 → **`sub` 為空 → 0/0**。
  - 後續輪次有時非零，是因**上一輪**寫入的 `validated_at` 可能已落在**新一輪週期起點**之前，屬**間接、延遲一輪**納入，與「本輪剛驗證」的直覺不一致。

- **產品／語意決策**（已採納；見 [`DECISION_LOG.md`](DECISION_LOG.md) **DEC-038**）：
  - 滾動 KPI 的 **「上界 now」語意**改為 **「本輪 `validate_once` 執行結束時刻（週期結束）」**，與 **`validated_at`＝驗證完成時間戳** 的定義對齊：同一輪內完成驗證的列，其 `validated_at` 應落在 **`[cycle_end − window, cycle_end]`** 內（在常規單調時鐘下），避免首輪與同輪 **0/0** 假陰性。
  - **不採**（本次明確排除）：為對齊上界而改為「全列共用週期起點時間戳寫入 `validated_at`」——會扭曲稽核／API **sync_ts** 語意（無法反映實際驗證完成時間）。

- **Goal**：
  1. 在 `validate_once` 內，於**即將**呼叫 `_rolling_precision_by_validated_at`、寫入 `validator_metrics`、以及（若需一致）相關 log 脈絡前，取 **`kpi_now_hk = datetime.now(HK_TZ)`** 作為滾動窗上界；**保留**週期開頭的 `now_hk` 供 `prune_validator_retention`、`finality_cutoff`、ClickHouse `fetch_end`、pending 篩選等**既有語意**（避免大範圍重算行為）。
  2. 更新 `_rolling_precision_by_validated_at` 之 **docstring／註解**：明載上界為 **「呼叫當下之 now（由呼叫端保證為週期結束或等價錨點）」**，避免未來誤用週期起點。
  3. **`validator_metrics.recorded_at`**：與 15m KPI 同一錨點時，建議使用 **`kpi_now_hk.isoformat()`**（與「快照觀測時刻」一致）；若產品要求 `recorded_at` 必須代表週期起點，則於實作 PR 中**顯式二選一並文件化**（預設建議跟隨 `kpi_now_hk`）。

- **Files（預期）**：
  - [`trainer/serving/validator.py`](../../trainer/serving/validator.py) — `validate_once`：`kpi_now_hk`；`_rolling_precision_by_validated_at(..., now_hk=kpi_now_hk)`；`_append_validator_metrics` 之 `recorded_at`（依上項決策）；註解。
  - 測試：新增或延伸 **unit / review_risks MRE** — 模擬「週期起點 < `validated_at` < KPI 上界」，斷言 15m（及必要時 1h）分母 **> 0** 且與預期 MATCH 計數一致；可抽一個小 helper 或 fixture 固定欄位最小集。

- **DoD**：
  - 冷啟後**首輪**若有非 PENDING 之 verified 列且 `validated_at` 在正常時鐘下為「剛剛」，則 **Cumulative Precision 兩行不再出現 `(0/0)`**（除非該窗內確實無 finalize 列）。
  - 既有 **`validated_at` 落窗**、tz-naive／mixed-tz 行為與 Task 4 SLO 測試**維持綠燈**；新增測試覆蓋本回歸根因。
  - `ruff` / 相關 `pytest` 通過。

- **Non-goals**：
  - 不改 `validate_alert_row` 內 **`validated_at`** 的產生方式（仍以每筆驗證當下 `now` 為準）。
  - 不重新定義 15m／1h 窗寬；不新增第二張 `validator_metrics` 列給 1h。

- **Rollback**：還原 `validate_once`／metrics 錨點變更；保留 docstring 澄清亦可獨立回退。

- **關聯**：Task 4（滾動 KPI / `validator_metrics`）；[`INCIDENT.md`](INCIDENT.md) 若曾記載「首輪 0/0」可補「根因與 Task 11」交叉連結。
...
| 2026-03-25 | **決策修訂（Task 4 / SLO）**：Validator 滾動 precision（15m/1h）改以 **`validated_at`** 落窗以支援即時監控（避免 `bet_ts`/`alert_ts` 在長期運行下多為歷史事件而使 15m/1h 變 `0/0`）；`validator_metrics` 與該 KPI 對齊；日誌標題建議標註 `by validated_at`。 |
...
| 2026-03-25 | Task 9 實作加固：safe int 解析、lookback cap、warn-once、extended-wait 納入 required_min；全量 tests/lint 綠燈（詳見 `STATUS.md`）。 |
| 2026-03-25 | Task 9B 規劃：新增 `No bet data` 二階段 targeted retry；補查窗改以每筆 `bet_ts`（即該筆 `payout_complete_dtm`）為基準，並加上每輪補查上限 50。 |
| 2026-03-25 | Task 9B 實作完成（Round 1）：targeted retry 上線，補查回填後同輪重驗；全量 tests/lint 與 focused mypy 通過；剩餘項目轉為穩定性/公平性加固。 |
| 2026-03-25 | Task 9B Round 2：query 容錯、retry 窗硬上界、round-robin 公平性、`existing_results_cache` DB-first；MRE 契約測試更新；見 `.cursor/plans/STATUS.md`。 |
| 2026-03-25 | Task 4（SLO）落地：Validator 15m/1h 滾動 precision 以 **`validated_at`** 落窗（含 tz-naive / mixed-tz 邊界修補）；新增 review_risks MRE 並全綠；`STATUS.md` 追加驗證指令與結果。 |
| 2026-03-26 | **Task 9C 規劃**：No-bet 補查以 **`bet_id` 錨定 TBET（方案 C）**；完整實作／DoD／開放問題自 [`INCIDENT.md`](INCIDENT.md) 遷入本檔；INCIDENT 僅保留事件敘事與對本 Task 之引用。 |
| 2026-03-26 | **Task 9C MVP 實作**：`fetch_bet_payout_times_by_bet_ids`、config 開關／chunk、`_fetch_bets_for_no_bet_rows` 合併與統計、單元測試 + Task9B MRE 延伸；見 `STATUS.md` 本日條目。 |
| 2026-03-26 | 新增 **Task 10** 規劃：Deploy 共用 SQLite 啟動鎖競爭 — 以 `threading.Event` 延後 validator 至 scorer **首輪 `score_once` 完成**；含 Design／Implementation／DoD、非目標（busy_timeout／以 alert 筆數閘門）、open questions 與 rollback。 |
| 2026-03-26 | **Task 10 實作**：`run_scorer_loop(..., first_cycle_done)`；`package/deploy/main.py`／`deploy_dist/main.py` 以 Event + `DEPLOY_VALIDATOR_START_TIMEOUT_SECONDS`（預設 600s）延後 validator；`tests/unit/test_scorer_first_cycle_event.py`；見 `STATUS.md` Task 10 條目。 |
| 2026-03-26 | **Task 10**：全量 pytest／ruff／mypy（trainer+package）驗證綠燈；Task 10 條目增 **Remaining / follow-ups**（`busy_timeout`、Review #1/#4、營運 DoD、wheel 版本）；`STATUS.md` 增「全 repo 驗證輪」。 |
| 2026-03-26 | 新增 **Task 11**：滾動 KPI 上界改為 **`validate_once` 週期結束時刻**（`kpi_now_hk`），與每筆 **`validated_at`** 一致；排除「改寫 `validated_at` 為週期起點」方案；含 Goal／Files／DoD／rollback。同步 **DEC-038** 於 `DECISION_LOG.md`。 |
| 2026-03-26 | **Task 11 實作**：`validate_once` 內 **`kpi_now_hk`** 供 `_rolling_precision_by_validated_at` 與 `validator_metrics.recorded_at`；docstring 補充；`tests/unit/test_validator_rolling_precision_alert_ts.py` 新增迴歸測；見 `STATUS.md` Task 11。 |

---

## Plan status snapshot（2026-03-26）

精簡對照（細節仍以各 Task 標題與內文為準）：

| Task | 狀態 |
|------|------|
| 1, 2, 4, 6, 9C, **10** | ✅ Done（**Task 10** 程式交付完成；**剩餘／後續**見該節 **Remaining / follow-ups**） |
| 3 | ⏳ 進行中（推論效能；Phase 5／Remaining 見該節） |
| 7 | ⏳ 進行中（chunk cache；R6 MVP Done 等見該節） |
| 5, 8 | 📝 Planned |
| **11** | ✅ Done（滾動 KPI 上界＝`kpi_now_hk`；見該節） |
| 9 | 🚧 In progress（含 9B 等；細節見該節與 `STATUS.md`） |

**全 patch 層級仍待收斂者（跨 Task）**：Task 3 之 p95／整合比對實測、Task 7 DoD 量化、Task 5/8 未開工、Task 9 剩餘子項、**Task 10 之 `busy_timeout` 與可選 Review 修補**（上表 Task 10 Remaining）。
