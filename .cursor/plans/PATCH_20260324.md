...
#### Phase 5 — ClickHouse：逐條 SQL 設計分析（文件、無實測要求）

- **交付物**：[`doc/task3_clickhouse_sql_analysis.md`](../../doc/task3_clickhouse_sql_analysis.md)。目前為 **占位**；後續逐條補全目的、欄位、時間窗、風險與瘦身建議。
- **DoD**：文件中每條相關 SQL 均有對應小節（完成後勾選清單）。
...
- **Phase 3（收斂）** — 尚待完成：
  - 產出 scorer 週期 p95 前後對照（固定資料窗/閾值）並記錄可接受浮點差範圍。
  - 補一組整合比對（同資料集）確認 alert 集合與 schema 下游相容。
- **Phase 5** — ClickHouse SQL 設計分析文件補全（`doc/task3_clickhouse_sql_analysis.md`）
...
- **Goal**:
  1. **降噪**：預設主控台僅保留「狀態變更／異常／精簡 KPI」在 `INFO`；高頻、每輪重複的 scorer／validator 訊息改為 `DEBUG`（必要時以環境變數或 `DEPLOY_LOG_LEVEL` 開啟）。
  2. **Validator KPI**：主控台除既有的 **15 分鐘（依 `alert_ts` 滾動窗）** `[validator] Cumulative Precision (15m window)` 外，新增 **`[validator] Cumulative Precision (1h window)`**；`validator_metrics` 寫入之 `precision/total/matches` 與 **15m 滾動 KPI** 對齊（語意與舊版「全表累積非 PENDING」不同，見下方 **Behavior change**）。
- **Scorer — 建議 `INFO` → `DEBUG` 的訊息**（[trainer/serving/scorer.py](../../trainer/serving/scorer.py)）:
  - `"[scorer] Window: ..."`、`No bets in window`、`New bets since last tick`、`No new bets to score`
  - `Feature rows`、`Rows to score`、`No usable rows…`、`No rated bets to score`
  - `No above-threshold alerts`、`Above-threshold rows`（若仍偏吵可維持 DEBUG）
  - `Suppressed … duplicate alerts`、`Alerts suppressed (already sent)`
  - 條件式：`Track LLM computed`、`player_profile PIT join`／unavailable、`Payout-age cap`（若運維仍依賴 cold-start 可暫留 `INFO`）
  - **保留 `INFO`**：啟動時模型載入摘要、`Emitted N alerts`、`WARNING`／`ERROR` 類。
- **Validator — 建議 `INFO` → `DEBUG` 的訊息**（[trainer/serving/validator.py](../../trainer/serving/validator.py)）:
  - `No alerts to validate`、`Alerts: … Pending: 0`、`pending_all: … min/max …` 診斷兩條、`… all too recent`、`Processing N alerts`
  - 每筆 **Finalizing … MATCH/MISS** 的逐筆 log（高流量時最大噪音源）
  - **保留 `INFO`**：`Cumulative Precision (15m/1h window)`、`Saved … validations`、`WARNING`／`ERROR`。
- **Deploy / Werkzeug**（[package/deploy/main.py](../../package/deploy/main.py)）:
  ...
  - 可選：`DEPLOY_LOG_LEVEL`（或沿用 `LOGLEVEL`）在 `basicConfig` 前設定根層級。
- **Behavior change（validator_metrics）**: 先前 `validate_once` 對 **所有** 已 finalize 列（retention 內）算一個 precision 並寫表；日誌卻標為「15m window」。**現行實作**改為依 **`alert_ts` 落於最近 15 分鐘（HK，`[now−15m, now]`）** 的子集計算 precision，並以**同一套定義**寫入 `validator_metrics`；另 **`1h`** 僅多一條 `logger.info`，不另寫第二筆列（避免 schema 變更）。
- **Files**:
  - [trainer/serving/validator.py](../../trainer/serving/validator.py) — `_rolling_precision_by_alert_ts`；雙行 Cumulative Precision log；`_append_validator_metrics` 與 15m 對齊。
  - （降噪待做）[trainer/serving/scorer.py](../../trainer/serving/scorer.py)、[package/deploy/main.py](../../package/deploy/main.py)。
- **DoD**:
...
### Task 7 — Step 6 Chunk Cache 命中率提升（程式層）— 📝 **Planned**（2026-03-24）

- **Context**:
  - 目前 Step 6 cache 為每月 chunk 的最終 Parquet + `.cache_key` sidecar；命中可略過 DQ/特徵/label/profile join 等重計算。
  - 現況 key 含 `bets_raw` hash、config hash、`profile_hash`、`feature_spec_hash`、`neg_sample_frac`，安全但偏保守；在資料順序變動、profile 輕微更新時容易 miss。
  - 現有命中分支仍會 `read_parquet` 取 row count，增加 hit path I/O 成本。
...
- **實作路線（升序）**:
  ...
  3. **R3：key 結構化與 miss reason 可觀測**  
     - `.cache_key` 改為結構化 payload（source/pipeline 分層），於 log/metrics 提供 `miss_reason`（如 data/spec/profile/config/neg_sample）便於後續優化。
  4. **R4：`profile_hash` chunk-scope 化**  
     - 由 run 級 `len+cols` 改為與 chunk 關聯的 fingerprint（例如以 `snapshot_dtm` 邊界 + canonical 子集 + 欄位版本），避免 profile 增量更新導致無關 chunk 全失效。
  5. **R5：local parquet source fingerprint（metadata-first）**  
     - local 模式優先使用檔案 metadata（mtime/size/row-group stats）作 source fingerprint，減少每次都先載入大量資料再判斷 key 的成本。
  6. **R6：兩段式快取（進階）**  
     - 新增中間層快取（如 DQ/normalize 後）與最終特徵快取分離；在 spec 或部分流程調整時可重用前段結果，降低全量重算機率。
