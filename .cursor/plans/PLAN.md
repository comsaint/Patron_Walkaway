...
| Task 7 | ⏳ In progress | **R1–R6 MVP** 已實作；**DoD（部分）**：`pipeline_diagnostics.json` 含 `step6_chunk_cache_*`；Review MRE：`test_task7_dod_chunk_cache_stats_review_risks_mre.py`；**仍待**：canonical_map／filter／schema／原子寫、合併驗證、`data_hash` 型別、DoD 分 Phase 計數（見 STATUS Review） |
