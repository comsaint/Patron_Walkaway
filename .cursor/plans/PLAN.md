...
overview: "Implement the full Patron Walkaway Phase 1 plan (v9) across 10 steps: 4 new modules, 6 refactored/updated modules, and a new tests/ directory — spanning approximately 4,000–5,000 lines of new/modified Python code."
todos:
  - id: install-deps
  ...
  - id: step0-config
    ...
    status: completed
  - id: step1-time-fold
    content: "Create trainer/time_fold.py: get_monthly_chunks(), get_train_valid_test_split() with [window_start, window_end) + extended_end semantics"
    status: completed
  - id: step2-identity
...
SQL patterns: both queries use `WITH deduped AS (ROW_NUMBER OVER PARTITION BY session_id ...)`.
...
```python
# Track A
def build_entity_set(bets_df, sessions_df, canonical_map, cutoff_times) -> ft.EntitySet:
    """Builds Featuretools EntitySet: t_bet → t_session → player."""
...
# Track B (vectorized, shared by trainer AND scorer)
...
def compute_table_hc(bets_df: pd.DataFrame, cutoff_time: datetime) -> pd.Series:
    """Vectorized: past TABLE_HC_WINDOW_MIN unique player_ids per table_id."""
...
```python
  def compute_micro_metrics(df, threshold): ...
  def compute_macro_by_visit_metrics(df, threshold): ...
  

```

- Per-visit at-most-1-TP dedup for evaluation (not for online scoring)
...
- Remove inline feature computation; replace with:
  - Track A: `featuretools.calculate_feature_matrix(saved_feature_defs, entityset)` with current poll time as cutoff
  - Track B: `from features import compute_loss_streak, compute_run_boundary, compute_table_hc`
...
- Visit dedup: use `gaming_day` column (already in t_bet/t_session)
...
├── test_trainer.py         ← sample_weight correctness, artifact bundle completeness
├── test_backtester.py      ← dual metrics, per-visit TP dedup evaluation
