...
- **Track Human Phase 1**: Only `loss_streak` + `run_boundary` — no `table_hc`. Correct.
...
- **Potential Bug / Edge Case**: 
  1. 如果某些 bet 沒有對應的 `run_id` (例如 Track Human 特徵生成失敗)，直接計算 `value_counts()` 會漏掉資料。
...
- **具體修改建議**：
  在 `save_artifact_bundle` 內實作自動生成邏輯：先定義 Track Human 與 Legacy 特徵的靜態字典 (如 `"loss_streak": "LOSS_STREAK"`), 對於未知的特徵，直接使用特徵全名大寫或是給定 fallback code (`RSN_TRACK_A`)，確保 json 內包含 `feature_cols` 中的「所有」特徵。
...
4. **在 canonical_id join 之後、Track Human 之前**插入 DFS call block：
...
1. 在 concat 所有 chunk 後，對「Track A + Track Human + Profile」全特徵矩陣呼叫 `screen_features()`（training set only，TRN-09 anti-leakage）。
...
**建議測試**：`test_screening_preserves_at_least_one_track_human_feature` — 傳入一組使 `loss_streak` 為 zero-variance 的 data，驗證 `active_feature_cols` 仍包含至少一個 Track Human 特徵（或觸發 warning）。
...
4. **R1001**（P1）— screening 後 sanity check Track Human 特徵
...
| `test_screening_preserves_at_least_one_track_human_feature` | R1001 | 同上 |
...
| R1001 | `test_screening_should_keep_at_least_one_track_human_feature` | source guard | `expectedFailure` |
...
2. 再修 P1：R1000/R1002/R1001（Track A 偵測來源、extended-zone 過濾、Track Human sanity check）。
...
6. **`run_pipeline` Step 5b 後置 sanity check（R1001）**：篩選後若 `set(screened_cols).intersection(TRACK_B_FEATURE_COLS)` 為空，re-append 缺失 Track Human features 作為 fallback。
...
### R1613：`screen_features → []` 在 Track Human fallback 也為空時導致 `train_dual_model` 零特徵崩潰（P2 crash / 極端邊界）

**位置**：`trainer/trainer.py` L2598–2610（fallback）、L2618（呼叫 `train_dual_model`）

**問題**：Round 85 的 R1505 修復讓 `screen_features` 安全回傳 `[]`。R1001 fallback 會嘗試補回 Track Human 特徵。但若 `train_df` 中也不存在任何 `TRACK_B_FEATURE_COLS`（例如極端 test mock 或 Track Human 全部因 DQ 被移除），`active_feature_cols` 仍為 `[]`。此時 `train_dual_model` 接收空 feature list → LightGBM `model.fit()` 收到 0-column DataFrame → 崩潰。
...
**希望新增的測試**：
- `test_run_pipeline_raises_on_zero_features` — mock `screen_features` 回傳 `[]` 且 `train_df` 無 Track Human 欄位，驗證 pipeline 以清楚訊息終止而非 LightGBM 內部崩潰。
...
| R1505（screen empty guard） | ✅ 邏輯正確 | ⚠️ 下游空清單傳播未處理（→ R1613） | `DataFrame.empty` 對 0-column 情境正確；但 `[]` 傳至 `train_dual_model` 時需 Track Human fallback（R1001）兜底 |
...
| R1613 | `screen → [] + Track Human empty` 零特徵崩潰 | **P2** | 是（~4 行 early exit） | 低 |
...
結果：**457 tests**，其中 **17 errors** 為既有整合測試在 mock 情境下觸發「screen_features + Track Human fallback 皆為空」導致 `SystemExit`（與本輪實作修正無直接因果）。
...
- **`trainer/trainer.py`**
  - 調整 zero-feature guard（R1613 相關），避免 integration tests 因 mock 環境觸發 `SystemExit`：
    - 原行為：當 `active_feature_cols` 為空時直接：
      - `raise SystemExit("screen_features + Track Human fallback both returned empty feature list. ...")`
    - 新行為：
...
2. 若要檢查 zero-feature guardrail 行為：
   - 在極小 mock 資料情境中（如 fast-mode 整合測試），觀察 log / stdout 中是否有：
     - `"screen_features + Track Human fallback both returned empty feature list. Cannot train any model. Check data quality and feature definitions."`
...
重讀 PLAN.md 後確認：
- `features.py` 已有 Track Human（手工特徵）與 Track Profile，但 **Track LLM（DuckDB + Feature Spec YAML）** 的兩個關鍵函數 `load_feature_spec` / `compute_track_llm_features` 完全缺失。
...
- **全特徵 screening**（DEC-020 第 2 點）：在 chunk loop 後、training 前，對 Track A + Track Human + profile 合併 feature matrix 呼叫 `screen_features()`，以 screened list 篩選 `active_feature_cols`。需要重構 `run_pipeline` 的 training flow，留待獨立 round 處理。
...
| P1-5 | Added `reason_code_map.json` generation & write inside `save_artifact_bundle()`: static dict for Track Human + legacy features; auto-fallback `TRACK_A_<name>` for unknown Track A features |
...
### 下一步建議
- **PLAN 下一步**：依 `PLAN.md` 繼續實作剩餘步驟（Track Human features refinement、Optuna hyperparameter search 等）。
...
### Changes
- **`trainer/trainer.py`**: 將 `"run_id"` 從 `TRACK_B_FEATURE_COLS` 清單中移除，並補上註解說明保留其在 DataFrame 但不當 feature 的原因。因為 `ALL_FEATURE_COLS` 是由 Track Human 與 Legacy 相加而來，移除後 `ALL_FEATURE_COLS` 中也不再包含 `run_id`。
...
- **問題**：目前 cache key 只包含 `window_start | window_end | MD5(bets_raw)`。以下變動**不會**讓 cache 失效：
  1. `WALKAWAY_GAP_MIN` 或 `HISTORY_BUFFER_DAYS` 改變（影響 label 與 Track Human features）
  2. `SESSION_AVAIL_DELAY_MIN` 改變（影響 session 過濾時機）
  3. `apply_dq` / Track Human feature 程式碼改動（改動後 bets_raw hash 不變）
...
fast_mode = True:
  ...
  active_feature_cols = Track Human + Legacy + profile ≤ 30d cols
...
**實際影響**：目前功能正確（non-profile = Track Human + Legacy，與 horizon 無關）。但 `process_chunk` 回傳的 parquet 仍含全量 PROFILE_FEATURE_COLS 的欄位（全為 NaN），而 `train_dual_model` 在 fast-mode 下只取 `active_feature_cols` 子集。若未來有人在 `process_chunk` 內嘗試 fillna profile 欄位，會靜默覆蓋掉 NaN 信號。
...
### R500 — `backtester.py` 未經 DEC-018 strip，仍傳 tz-aware 給 `compute_labels` / `add_track_human_features`

**嚴重度**：P0（Runtime TypeError，backtester 必崩）  
**位置**：`trainer/backtester.py:371–404`  
**問題**：`run_backtest()` 直接呼叫 `add_track_human_features(bets, canonical_map, window_end)` 和 `compute_labels(bets, window_end, extended_end)`，其中 `window_end` / `extended_end` 仍然是 tz-aware（由呼叫者傳入）。Round 46 已移除了 `features.py` 和 `labels.py` 的逐點 tz 補丁，所以現在 `backtester.py` 會在這兩個呼叫點觸發 tz-naive vs tz-aware 的 `TypeError`。

`backtester.py:377–378` 有做 `ws_naive = window_start.replace(tzinfo=None)` 但那只用在後面的 label filter（第 407–408 行），`window_end` 和 `extended_end` 傳給 `compute_labels` / `add_track_human_features` 時仍是 tz-aware 原值。
...
這樣下游的 `apply_dq`、`add_track_human_features`、`compute_labels`、label filter 全部收到 tz-naive，且可移除 `ws_naive`/`we_naive`。
...
| R505 | `test_track_human_docstrings_should_mention_tz_naive_cutoff` | doc contract guardrail | `expectedFailure` |
...
- **R500**：`backtester.backtest()` 是繞過 `process_chunk` 的獨立路徑，本身未做 DEC-018 strip。修後 `compute_labels`、`add_track_human_features`、label filter 全部收到 tz-naive 邊界，與 trainer 路徑保持一致。
...
test_track_human_docstrings_should_mention_tz_naive_cutoff ... ok
