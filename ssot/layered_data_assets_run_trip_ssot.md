...
| **Run（連續下注段）v1** | 同一 `player_id` 之下注序列中，**相鄰兩筆 bet 之事件時間間隔 ≤ 30 分鐘**者屬同一 run；超過 30 分鐘則開新 run。 |
| **Trip（行程段）v1** | 同一 `player_id`，若上一筆 bet/run 結束後出現 **3 個完整 `gaming_day` 皆無任何 bet**，則該 trip 才可關閉，下一次 bet 視為**新 trip** 之起點。例：最後 bet/run 結束於 Jan 1，必須 Jan 2、Jan 3、Jan 4 三個完整 gaming day 皆無 bet，該 trip 才在 Jan 4 後可定版關閉。 |
| **階層關係** | **`trip` 為 `run` 之聚合；`run` 為 `bet` 之壓縮層。** Trip 級特徵應優先由 run 級可聚合統計得出；僅在契約明列時才允許回掃 bet。 |

### 3.1 設計意圖（非規範細節）

- Run 可**跨 `gaming_day`**；因此 run 之物化分區策略須與「單日 trip 分區」分離設計（見 §5.2）。
...
| 實體 | 用途 |
|------|------|
| **`run_fact`** | 每個 run 一列；保存 run 級可加總或可合併之統計與時間邊界（供 L2 與 trip 聚合）。 |
| **`trip_fact`** | 每個 trip 一列；由 `run_fact`（及必要之邊界 metadata）聚合而來。 |
| **`run_day_bridge`（或等價名稱）** | Run 與「bet 所屬 `gaming_day`」之對照，支援**跨日 run** 之日粒度增量掃描，以及判斷「某日資料變動可能影響哪些 run」之影響範圍。 |
...
因 run 可能跨多個 `gaming_day`，須同時滿足：

1. **`run_fact` 主分區**：建議以 **`run_end_gaming_day`**（run 內最後一筆 bet 所屬 `gaming_day`）作為**每列唯一**分區鍵，避免同一 run 被多日重複主存。
2. **`run_day_bridge`**：以 **`bet_gaming_day`** 分區，記錄該 run 與各日之交會列（或等價壓縮表示），供增量作業掃描「某日變動可能影響哪些 run」。

> **禁止**：僅以單一 `gaming_day` 切 run 導致跨日 run 語義斷裂，除非另立 **definition_version** 並明載於 manifest。
...
- **遲到或上游修正**：觸發**受控重算**時，範圍由「影響分析」（例如經 `run_day_bridge` 與分區鍵）決定，而非由固定日曆窗口單獨決定。
...
| **`trip_id`** | `hash(player_id, trip_start_gaming_day, trip_definition_version, source_namespace)` |
...
| ID | 決策 |
|----|------|
| **LDA-001** | run/trip 定義採 **v1**（§3），後續變更須升 `definition_version`。 |
| **LDA-002** | 玩家主鍵採 **`player_id`**；本層**不使用 `canonical_id`**。 |
| **LDA-003** | Trip 分區以 **`gaming_day`** 為標準。 |
| **LDA-004** | Run 跨日採 **`run_fact` + `run_day_bridge`** 雙軌（§5.2）。 |
