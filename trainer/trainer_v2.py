...
def build_labels_and_features(
    ...
    # Drop back bets and seat_id == 0 before feature engineering
    ...
    before_filter = len(bets_df)
    bets_df = bets_df[(bets_df["is_back_bet"] != 1) & (bets_df["position_idx"] != 0)]
    if before_filter != len(bets_df):
        print(f"[trainer] Skipped {before_filter - len(bets_df)} bets (back bets or seat_id=0)", flush=True)
