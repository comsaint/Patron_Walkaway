...
# Issue #12 PR-12.2: data ingress helpers + column lists live in
# ``trainer.training.data_sources`` so trainer.py only orchestrates. The
# names below are imported (and re-exported) to preserve external call sites
# such as ``parallel_lda_mvp.trainer_bridge_mvp`` and the wider test suite.
...
# Issue #12 PR-12.4: pure metric-evaluation helpers now live in
# trainer.training.metrics_eval. Re-exported below so historic call sites
# continue to find them under trainer.training.trainer.
from trainer.training.metrics_eval import (  # noqa: E402
