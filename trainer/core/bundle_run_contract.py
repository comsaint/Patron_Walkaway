...
``training_metrics.json`` when *bundle_root* is set and the file is readable;
otherwise :data:`trainer.core.config.SELECTION_MODE`.
...
def read_bundle_run_contract_block(
    ...
    *,
    ...
    if bundle_root is not None:
        tm_path = Path(bundle_root).resolve() / "training_metrics.json"
        if tm_path.is_file():
            try:
            ...
            if isinstance(tm, dict):
                ...
                if raw_mode is not None:
                    ...
                    if cand:
                        sel = cand
                        src = "artifact_training_metrics.json"
