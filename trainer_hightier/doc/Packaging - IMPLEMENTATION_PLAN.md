...
- 建包或 deploy preflight 必須驗證 **model feature supplyability**：
  ...
  - `source: fe_derived` 必須依 frozen registry 的 cadence / `allowed_training_supplier` 分派，不可再要求單一 legacy `fe_derived_parquet` 同時供應 short-term 與 mid-term。
  - 若模型含 `fe_derived` 類欄位，但 bundle / wheel / runtime 沒有對應 cadence-aware 供應路徑，或 mid-term **freshness** 未達 SLA，必須 fail-fast，不可等到 scorer 第一輪才由 `assert_features_ready` 爆錯。
...
- 現況欄位對照（治理用，非 exhaustive）：
  ...
  - `fe__*` 中 `<24h` / event-level 欄位（`fe_derived` + `allowed_training_supplier=short_term_pit_builder`）→ short-term
  - `fe__*` 中 1d–30d snapshot / composite 欄位（`fe_derived` + `allowed_training_supplier=mid_term_daily_snapshot`）→ mid-term
...
| **snapshot_parquet** | 訓練凍結或低頻全量/增量快照（如 mid-term daily snapshot、slow 180d） | mid-term / long-term |
...
- D-008（已定）：**Time-horizon（term）** 與 **supplier** 分離治理（見上節「時間視野分類」）。
  ...
  - `feast_slow_180d` 為 **long-term**，**不是** mid-term；`fe_derived` 必須依 cadence split，不能整個 family 一律視為 mid-term。
...
- M8：feature supplyability gate 生效；含 `fe_derived` 的模型若缺 cadence-aware supplier 或 mid-term freshness 過期，會在建包/preflight 階段失敗，而非 scorer 第一輪失敗。
