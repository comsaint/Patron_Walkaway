...
- 正式 bundle 不只要 `/health` 可用，也必須證明 `model.pkl.feature_columns` 的每個欄位在 runtime 有供應者（ClickHouse raw、online builder、或 bundled parquet）。
...
| 模型特徵供應 | 目前主要靠 scorer 執行時 `assert_features_ready` 才發現缺欄；`fe_derived` 類欄位可能 build/boot 成功但 scoring 失敗 | 建包或 deploy preflight 建立 feature supplier matrix；任何 `model.feature_columns` 無 runtime supplier 時 fail-fast |

## 與 implementation plan 里程碑對照（M1–M8）
...
| M7 | Phase 4：`prediction_log.db` 全量 scored rows 可寫入與查詢 |
| M8 | Phase 5：feature supplyability gate 生效；含 `fe_derived` 但無 supplier 的模型不可進 production |
...
- [ ] feature supplyability 仍未完整納入 release gate：需在建包或 deploy preflight 前確認 `model.pkl.feature_columns` 全部可由 runtime 供應；`/health` 不代表 scorer ready。
...
| P1-5 | 建立 **feature supplier matrix**：讀 `model.pkl.feature_columns` + frozen registry，將欄位分派到 `baseline_model` / `feast_trial_1h` / `feast_slow_180d` / `fe_derived` 供應者 | P1-1, P1-3 | 建包輸出或 preflight log 有 supplier summary；任何欄位無 supplier 會 fail-fast |
| P1-6 | 明確 `fe_derived` production 策略：serving-safe online builder 或 bundled `fe_derived_*.parquet`；未完成前，含 `source: fe_derived` 的 production 模型不得通過 gate | P1-5 | 含 `fe_derived` 且無 supplier 的模型建包/preflight 失敗，錯誤列出缺失欄位與 source |
...
| P2-5 | deploy preflight 納入 feature readiness：啟動時或 scorer `--once` 前，列出 model features 的 supplier summary；不再只檢查檔案存在 | P1-5, P2-1 | 缺欄會在 preflight 或 scorer smoke 以 actionable error 阻擋，而非第一輪 production scoring 才炸 |
...
| P3-4 | Scorer feature readiness smoke：在無 repo bundle 環境跑一輪 scorer 路徑至 `assert_features_ready` / `predict_proba` 前後 | P2-5 | 證據包含 `model.feature_columns` 全可供應；若無 live CH，使用固定 fixture / mock adapter 仍需覆蓋完整 feature builder 鏈 |

### Phase 4：Prediction Log parity

| ID | 任務 | 依賴 | Definition of done |
|----|------|------|-------------------|
| P4-1 | 初始化 `prediction_log.db` 並在 scorer 每輪 `predict_proba` 後、alert 篩選前寫入全量 scored rows | P2-1 | DB schema / index 建立；寫入失敗策略明確 |
| P4-2 | 在 no-repo smoke 或 fixture smoke 中驗證 prediction log 可讀回全量 predictions | P4-1, P3-4 | 非僅 alerts；可查 `model_version`、`score`、`margin`、`is_alert` |

### Phase 5：Feature supplyability parity

| ID | 任務 | 依賴 | Definition of done |
|----|------|------|-------------------|
| P5-1 | 定義 runtime supplier registry（raw / online / bundled parquet）與每個 registry `source` 的映射規則 | P1-5 | `baseline_model`、`feast_trial_1h`、`feast_slow_180d`、`fe_derived` 均有明確策略 |
| P5-2 | 補 build-time gate：`source` 不能等同供應保證；必須有可執行 function 或 packaged artifact | P5-1 | 新增/變更 feature 時，未同步 serving / artifact 會在建包階段失敗 |
| P5-3 | 補 runtime smoke：以 bundle 內 wheel、manifest、mapping、slow/optional feature parquet 跑完整 feature builder 鏈 | P5-2, P3-4 | `assert_features_ready` 通過並進入 `predict_proba`；失敗訊息列 source 與缺失欄位 |
| P5-4 | 決策並落地 `fe_derived` 供應策略 | P5-1 | 兩選一：serving-safe online builder，或 `fe_derived_*.parquet` bundled merge；未完成前含 `fe_derived` 的模型不可標 production-ready |
...
flowchart TD
  ...
  P2[P2 runtime 與設定]
  P3[P3 乾淨機驗收]
  P4[P4 prediction log]
  P5[P5 feature supplyability]
  P0 --> P1
  P1 --> P2
  P2 --> P3
  P3 --> P4
...
| F | feature supplyability gate（待補） | `model.pkl.feature_columns` × frozen registry × runtime suppliers |
...
- [ ] Scorer feature readiness smoke 通過：`model.feature_columns` 全部可由 runtime supply chain 產出，且可進入 `predict_proba`。
...
- [ ] 跑一輪 scorer readiness（live CH shadow 或固定 fixture/mock adapter）：不得只停在 API alive；需覆蓋 `attach_*` feature builder、slow/feature parquet merge、`assert_features_ready`、`predict_proba`。
...
| model feature 無 runtime supplier | `assert_features_ready` 缺 `fe__*` 或新 feature 欄位 | 建包/preflight 以 supplier matrix 阻擋；短期用 serving-compatible model，長期補 online builder 或 bundled parquet |
| 只驗 `/health` 導致 scorer 首輪才失敗 | API 正常但 `python main.py --mode all` 後 scorer traceback | Release gate 加 scorer readiness smoke；`/health` 只代表 API alive |
| 大 parquet runtime 全檔載入 | 目標機 OOM 或 scorer 每輪卡住 | runtime merge 應按當輪 `bet_id`/entity set 過濾讀取；schema gate 只讀 schema |

---

**下一步（僅提醒）**：Gap 清單全勾後，本 working plan 的 Phase 2–5 即與 implementation plan「Standalone Runtime Bundle（無 repo、預設 PyPI 安裝 + feature supplyability）」正式對齊；此前 production gate 以「過渡部署 + 顯式環境前提」註記於變更單。
