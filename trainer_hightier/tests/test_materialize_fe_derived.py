...
def test_compute_fe_derived_chase_loss_and_clock_features() -> None:
    """Chase-loss streak/ratios and clock context are PIT-safe on bounded pool."""
    hk = "Asia/Hong_Kong"
    t0 = pd.Timestamp("2025-06-02 18:00:00", tz=hk)
    pool = pd.DataFrame(
        {
            "bet_id": [100.0, 200.0, 300.0, 400.0],
            "player_id": [10, 10, 10, 10],
            "canonical_id": ["c10", "c10", "c10", "c10"],
            "session_id": [1, 1, 1, 1],
            "table_id": [1, 1, 1, 1],
            "game_id": [1, 1, 1, 1],
            "gaming_day_event": pd.to_datetime(["2025-06-02"] * 4),
            "payout_complete_dtm": [
                t0,
            ...
            "wager": [100.0, 100.0, 250.0, 50.0],
            "payout_odds": [2.0, 2.0, 2.0, 2.0],
            "casino_win": [10.0, 10.0, 10.0, -5.0],
            "theo_win": [5.0, 5.0, 5.0, 5.0],
        }
    )
    got = compute_fe_derived_features_from_pool(pool, pd.Series([400.0]))
    row = got.iloc[0]
    assert float(row["fe__outcome__consecutive_loss_streak"]) == pytest.approx(0.0)
    assert float(row["fe__clock__hour_of_day"]) == pytest.approx(18.0)
    assert float(row["fe__clock__is_late_night"]) == pytest.approx(0.0)
    assert float(row["fe__clock__is_weekend"]) == pytest.approx(0.0)
    assert float(row["fe__outcome__loss_then_double_ratio__w1h"]) == pytest.approx(1.0 / 3.0)
    assert float(row["fe__outcome__wager_after_loss_step_ratio__w1h"]) == pytest.approx(1.75)

    got_loss = compute_fe_derived_features_from_pool(pool, pd.Series([300.0]))
    loss_row = got_loss.iloc[0]
    assert float(loss_row["fe__outcome__consecutive_loss_streak"]) == pytest.approx(3.0)


def test_compute_fe_derived_stake_escalation_features() -> None:
    """Last-3 vs prior-3 ratio and wager trend slope over short-term windows."""
    hk = "Asia/Hong_Kong"
    t0 = pd.Timestamp("2025-06-02 18:00:00", tz=hk)
    wagers = [100.0, 100.0, 100.0, 150.0, 200.0, 250.0]
    pool = pd.DataFrame(
        {
            "bet_id": [float(i) for i in range(100, 700, 100)],
            "player_id": [10] * 6,
            "canonical_id": ["c10"] * 6,
            "session_id": [1] * 6,
            "table_id": [1] * 6,
            "game_id": [1] * 6,
            "gaming_day_event": pd.to_datetime(["2025-06-02"] * 6),
            "payout_complete_dtm": [t0 + pd.Timedelta(minutes=i) for i in range(6)],
            "wager": wagers,
            "payout_odds": [2.0] * 6,
            "casino_win": [0.0] * 6,
            "theo_win": [1.0] * 6,
    ...
    got = compute_fe_derived_features_from_pool(pool, pd.Series([600.0]))
    row = got.iloc[0]
    assert float(row["fe__stake__wager_last3_vs_prior3_ratio__w1h"]) == pytest.approx(200.0 / 100.0)
    assert float(row["fe__stake__wager_trend_slope__w1h"]) > 0.0
