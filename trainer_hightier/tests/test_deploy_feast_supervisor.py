...
def test_feast_refresh_supervisor_clears_store_cache_on_ok_verdict(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Successful refresh must invalidate cached FeatureStore (registry mtime unchanged by materialize)."""
    cfg = _cfg(tmp_path)
    init_feature_state_db(cfg.feature_state_db_path)
    (tmp_path / "feast_repo").mkdir()
...
def test_feast_refresh_supervisor_skips_cache_clear_on_non_ok_verdict(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Non-ok refresh verdict must not drop the scorer's cached FeatureStore."""
    cfg = _cfg(tmp_path)
    init_feature_state_db(cfg.feature_state_db_path)
    (tmp_path / "feast_repo").mkdir()
    cache_cleared: list[bool] = []
