...
def test_val_p1hr_precision_returns_none_when_missing() -> None:
...
def test_precision_to_pp_and_delta_p1hr_pp() -> None:
    assert precision_to_pp(0.25) == pytest.approx(25.0)
    assert precision_to_pp(None) is None
    base = {"val_op_precision_at_1p0_alerts_per_hour": 0.20}
    arm = {"val_op_precision_at_1p0_alerts_per_hour": 0.25}
    assert delta_p1hr_pp(base, arm) == pytest.approx(5.0)


def test_fold_metrics_from_report_populates_pp() -> None:
    report = {
        "val_op_precision_at_1p0_alerts_per_hour": 0.18,
        "val_ap": 0.31,
        "step5_val_recall_at_pick": 0.12,
    }
    metrics = fold_metrics_from_report(report, fold_idx=2, arm_id="baseline")
    assert metrics.fold_idx == 2
    assert metrics.arm_id == "baseline"
    assert metrics.val_p1hr_precision == pytest.approx(0.18)
    assert metrics.val_p1hr_precision_pp == pytest.approx(18.0)
    assert metrics.val_ap == pytest.approx(0.31)
    assert metrics.val_recall_at_pick == pytest.approx(0.12)
